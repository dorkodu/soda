...
class Lucid {
    local(value, component) {
        let setState = (value) => { component.state = value; component.update(); };
    ...
    _update(dom, element, oldElement) {
        ...
        for (let i = 0; i < element.children.length || i < dom.childNodes.length; ++i) {
            ...
            if (typeof element.children[i].tag === "function") {
                ...
                if (dom.childNodes[i] === undefined) {
                    ...
                    dom.insertBefore(container.firstChild, dom.childNodes[i]);
                }
            }
            else if (typeof element.children[i] === "object") {
                if (dom.childNodes[i] === undefined) {
                ...
                this._update(dom.childNodes[i], element.children[i], oldElement.children[i]);
            }
            else {
                if (dom.childNodes[i] === undefined) {
