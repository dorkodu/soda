  ...
  hooks: {
    connected: function () {
      ...
      for (let i = 0; i < elements.length; ++i) {
        lucid.render(this.dom, Circle, i, { element: elements[i], id: i });
      }
    }
  }
});

const Circle = lucid.component({
  attributes: function () { return { element: undefined, id: undefined } },
  methods: {
    getStyle: function () {
      return `top:${10 * Math.sin(n / 10) + "px"};left:${10 * Math.cos(n / 10) + "px"};background:rgb(0,0,${n % 255})`;
    },
    getCount: function () {
      return n;
    }
...
Benchmark.Framework.Lucid = {
  ...
  loop() {
    Promise.resolve().then(() => {
      update();
      for (let i = 0; i < Benchmark.number; ++i) {
        lucid.instance(Circle, i).attribute("element", elements[i]);
      }
    })
  }
}